...
userNameOfAccount = "markemerson13"
...
def automate_twitter_data_scraping(user_account, scroll_amount, scroll_delay, max_scrolls):
    ...
    with sync_playwright() as p:
        ...
        while scrolls < max_scrolls:
            ...
            # Scroll into view to load more tweets
            try:
                page.wait_for_selector('div[data-testid="tweet"]:nth-child(15)')
            except:
                break

            new_tweet_count = len(page.query_selector_all('div[data-testid="tweet"]'))
            if new_tweet_count == last_tweet_count:
                break  # No new tweets loaded, exit the loop
            last_tweet_count = new_tweet_count
            scrolls += 1
